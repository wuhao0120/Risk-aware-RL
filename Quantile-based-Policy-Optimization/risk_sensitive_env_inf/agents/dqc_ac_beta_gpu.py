...
from envs import RiskSensitiveVecTorch                  # 全 GPU、B 路并行的【无状态 continuing】向量化环境 (∞-horizon)
from .common import DistributionalCritic, lr_lambda, indicator  # ∞-horizon setting 内共享件 (见 common.py)
...
class DQCACBetaGPU(object):
    ...
    def __init__(self, args, env):
        ...
        # critic step 增广: 在【∞-horizon / continuing】设定下【关闭】(默认 False)。
        # 本环境无状态、continuing → 回报-to-go G_t 与 t 无关 ⇒ 价值平稳 V(s)≡V ⇒ step-blind critic 即正确。
        # (有限步版才需要 step 特征 t/n 来区分 t-依赖的 G_t; 这里恒为 False, _aug 原样返回, 不拼 step。)
        self.critic_step_feature = bool(getattr(args, 'critic_step_feature', False))  # ∞-horizon: False (step-blind, 价值平稳)
        ...
        self.n = self.vec_env.n                                            # 一段 rollout 步长 T (= env.n, 截断长度)
        self.state_dim = int(np.prod(env.observation_space.shape))         # 状态维度 (无状态, =1)
        ...
        # -------------------- wandb (∞-horizon: 独立 project 'risk_sensitive_inf', 不与有限步 RiskSensitiveEnv 混在一起) --------------------
    ...
    def _rollout_vec(self):
        ...
        # ---- n-step TD 目标预计算 (∞-horizon / continuing: 截断点 bootstrap, 无终止) ----
        # 对 transition (t, env): 累计回报 = Σ_{k=0}^{min(N,n-t)-1} γ^k r_{t+k} (序列末自动截断);
        #   continuing 任务【恒 bootstrap】(boot_mask≡1): 第 n 步是【截断 truncated】而非终止 →
        #   bootstrap 态 s_{t+N} (越界则 clamp 到截断次态 s_T) 给出平稳价值 V; dones≡0 (不当作 terminal)。
        #   N=1 时即【精确】1-step TD: y = r + γ·ψ̄(s_{t+1}) (s_{t+1} 末端取 s_T)。
        #   (N>1 仅末 N-1 个 transition 的 bootstrap 折扣视界有可忽略近似, 且 N>1 已知不稳, 保持 N=1。)
        Ns = self.n_step
        ...
        # 扩展状态序列以含【截断次态 s_T】(= 最后一个 next_state, [B,sd]) → 索引 n 可达, 使末端 transition 可 bootstrap
        S_ext = torch.cat([Smat, S2[-1].unsqueeze(0)], dim=0)  # [n+1, B, sd]; 索引 n = s_T
        boot_idx = torch.clamp(t_ar + Ns, max=n)               # [n] bootstrap 态 s_{t+N} 索引 (可达 s_T=索引 n)
        boot_states = S_ext[boot_idx].reshape(n * B, -1)       # [n·B, sd]  s_{t+N} (含截断点 s_T)
        boot_mask = torch.ones(n * B, dtype=torch.float32, device=self.device)  # continuing: 恒 bootstrap (无终止)
        nstep_reward = nstep_rew.reshape(n * B)                # [n·B]  Σ_{k<m} γ^k r_{t+k}
        # step 特征仅在 critic_step_feature=True 时用; ∞-horizon 恒 False → _aug 原样返回, 下两量不进网络 (仅占位)
        steps = t_ar.unsqueeze(1).expand(n, B).reshape(n * B).float()       # [n·B]  当前步 t (未用)
        boot_steps = boot_idx.unsqueeze(1).expand(n, B).reshape(n * B).float()  # [n·B]  bootstrap 态步数 (未用)

        # done: continuing 任务无终止 ⇒ 恒 0 (截断点也 bootstrap, 不当作 terminal)
