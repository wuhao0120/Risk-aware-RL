...
def base_args(algo):
    ...
    a.algo_name = algo
    a.q_alpha = 0.25
    a.gamma = 0.9                 # ∞-horizon: 有效视界 1/(1-γ)=10, T=100≫10 → 价值平稳, 尾项 γ^100≈3e-5
    a.env_n = 100                 # 一段 rollout 截断长度 T (continuing)
    a.log_interval = 50
    a.est_interval = 100
    a.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    a.wandb_dir = None
    a.wandb_project = 'risk_sensitive_inf'    # ∞-horizon 独立 wandb project (不与有限步混)

    if algo == 'DQCACBetaGPU':
        a.num_envs = 512
        a.num_iterations = 1500        # critic 校准 + mean 收敛需 ~1200+
        a.beta = 0.90                  # Abel 风险折扣; 也天然下压 budget 在末端 blow-up 的 transition (β^t→0)
        a.quantile_threshold = 6.0     # ∞-horizon 工作点(推荐): 收敛 r≈0.70/mean≈15.5/Q≈6.2/P≈0.24 (QPO 保守解 mean≈10.8)
        # 注: 此环境【q 越大约束越紧】; 可行需 q ≤ 天花板 Q0.25≈7.45 (_sweep.py 实测@r0.1); q=8 不可行 → λ winding up + mean 塌到 10
        a.critic_step_feature = False  # ∞-horizon 无状态 → 价值平稳 → step-blind critic 正确
        ...
        a.quantile_threshold = 6.0     # ∞-horizon 工作点 (与 DQCACBetaGPU 同 q, 公平对比)
        a.init_std = float(np.sqrt(1e-1))
...
def main():
    ...
    env = RiskSensitiveEnv(n=int(getattr(args, 'env_n', 100)))   # ∞-horizon: 无状态 continuing, 截断长度 T
    AgentCls = {'DQCACBetaGPU': DQCACBetaGPU, 'QCPOGPU': QCPOGPU}[algo]
    ...
    if algo == 'DQCACBetaGPU':
        ...
        verdict(mean_e > 10.5, 'mean>QPO(~10)', f'mean={mean_e:.3f} (∞-h: 约束最优应显著>QPO保守解≈10)')
        ...
        verdict(lam_mean_2nd < 0.5 * args.lambda_max, 'lambda not runaway',
                f'lam_mean_2nd={lam_mean_2nd:.3f} lam_min_2nd={lam_min_2nd:.3f} lam_last={lam_last:.2f} '
                f'(near-0/投影边界 ok per memory; bad=runaway/pegged {args.lambda_max})')
        verdict(np.isfinite(nl_last) and nl_last < 50, 'loss not exploded', f'norm_loss={nl_last:.3f}')
        verdict(np.isfinite(rstd_last) and rstd_last < 40, 'variance stable', f'return_std={rstd_last:.3f}')
