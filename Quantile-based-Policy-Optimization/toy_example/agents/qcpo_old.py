...
def golden_section_search(func, a, b, epsilon=1e-3, max_iter=100):
    """
    黄金分割搜索算法，用于优化凸对偶函数L(λ)
    
    参数:
        func: 目标函数（凸函数）
        a: 搜索区间下界
        b: 搜索区间上界
        epsilon: 容忍度（搜索区间长度阈值）
        max_iter: 最大迭代次数
...
class QCPO(object):  # QPO类封装了QPO算法,负责管理整个训练过程
    ...
    def __init__(self, args, env):
        ...
        # QCPO特有参数
        self.outer_interval = args.outer_interval   # 外层循环间隔
        self.lambda_eps = args.lambda_eps           # 黄金分割搜索的容忍度
        self.lambda_min = args.lambda_min           # 黄金分割搜索的下界
        self.lambda_max = args.lambda_max           # 黄金分割搜索的上界
        ...
        # 初始化拉格朗日乘子λ
        self.lambda_dual = torch.tensor(0.0).to(self.device)  # 初始化lambda为0, 在update_outer()中更新
        
        # 用于黄金分割搜索的历史数据
        self.disc_rewards_buffer = []               # 一个python列表，用来保存最近outer_interval个trajectory的reward
                                                    # 在dual_function()中被使用，使用Monte Carlo方法估计对偶函数中的期望E和分位数Q
    ...
    def train(self):
        ...
        inner_step_counter = 0      # 内层步数计数器,用来记录内层循环的步数,如果内层步数达到outer_interval,就进行外层循环
        ...
        for i_episode in range(self.max_episode + 1):
            ...
            # 内层循环：固定λ，更新(θ, Q)
            self.update_inner()                                     # 一个episode结束之后,用整条trajectory数据做一次梯度更新
            ...
            self.disc_rewards_buffer.append(disc_epi_reward)        # 将当前episode的cumulative return添加到disc_rewards_buffer列表中,用于估计对偶函数中的期望E和分位数Q
            ...
            inner_step_counter += 1                                 # update_inner被调用一次, inner_step_counter加1
            
            # 外层循环：每隔一定步数，使用黄金分割搜索优化λ
            if inner_step_counter >= self.outer_interval and i_episode > 0:
                self.update_outer()                                 # 如果inner_step_counter达到outer_interval,就进行外层循环
                inner_step_counter = 0                              # 进行外层循环之后,将inner_step_counter重置为0,准备下一个外层循环
                # 清空缓冲区
                self.disc_rewards_buffer = []                       # 在外层更新lambda时，计算对偶函数值要用到disc_rewards_buffer列表中的数据，更新完后清空准备下一个外层循环
            ...
            if i_episode % self.log_interval == 0 and i_episode != 0:
                ...
                print(f'Epi:{i_episode:05d} || model Updated with lr:{self.scheduler.get_last_lr()[0]:.2e} '
                      f'q_lr:{self.q_scheduler.get_last_lr()[0]:.2e} q_est:{self.q_est.item():.03f}\n')
    ...
    def dual_function(self, lambda_val):
        """
        计算对偶函数 L(λ) 用于黄金分割搜索
        L(λ) = max_θ {E[U(τ)] - λ{q - Q(α;θ)}} = max_θ {E[U(τ)] + λ(Q(α;θ) - q)}
        
        这里使用最近一段时间的轨迹数据
        来估计对偶函数的值
        """
        if len(self.disc_rewards_buffer) == 0:
            return 0.0
        
        # 使用缓冲区中的数据估计期望回报和分位数
        rewards = np.array(self.disc_rewards_buffer)
        expected_return = np.mean(rewards)
        quantile_est = np.percentile(rewards, self.q_alpha * 100)
        
        # 计算对偶函数值
        # L(λ) = E[U(τ)] + λ(q - Q(α;θ))
        dual_val = expected_return + lambda_val * (quantile_est - self.quantile_threshold)
        
        return dual_val

    def update_outer(self):
        """
        外层循环：使用黄金分割搜索优化拉格朗日乘子λ
        根据算法伪代码第5步
        """
        # 使用黄金分割搜索寻找最优λ
        optimal_lambda = golden_section_search(
            func=self.dual_function,
            a=self.lambda_min,
            b=self.lambda_max,
            epsilon=self.lambda_eps
        )
        
        # 更新λ
        self.lambda_dual = torch.tensor(optimal_lambda).to(self.device)
        
        # print(f'Outer loop: λ updated to {optimal_lambda:.4f}')
