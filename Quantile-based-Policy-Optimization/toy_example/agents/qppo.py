...
from utils import Memory, Actor, Critic
...
class QPPO(object):
    ...
    def __init__(self, args, env):
        ...
        self.old_actor = Actor(state_dim, action_dim, args.init_std)
        self.old_actor.load_state_dict(self.actor.state_dict())
        self.critic = Critic(state_dim+1, args.emb_dim)

        self.optimizer = Adam(list(self.actor.parameters()) + list(self.critic.parameters()), 1., eps=1e-5)
        ...
        self.q_optimizer = Adam([self.q_est], 1., eps=1e-8)
    ...
    def train(self):
        ...
        for i_episode in range(self.max_episode+1):
            ...
            wandb.log({'disc_reward/raw_reward': disc_epi_reward}, step=i_episode)
    ...
    def update(self):
        ...
        count = 0
        actor_loss_aver, critic_loss_aver, dist_entropy_aver = 0, 0, 0
            ...
            for l in shuffle_l:
                ...
                for i in shuffle_epi:
                    ...
                    if batch_len >= self.mini_batch:
                        ...
                        actor_loss_aver += actor_loss.detach()
                        critic_loss_aver += critic_loss.detach()
                        dist_entropy_aver += dist_entropy.detach()
                        count += 1
