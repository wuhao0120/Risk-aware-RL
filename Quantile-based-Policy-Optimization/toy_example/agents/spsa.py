...
class SPSA(object):
    ...
    def __init__(self, args, env):
        ...
        self.actor_plus = Actor(state_dim, action_dim, args.init_std)
        self.actor_minus = Actor(state_dim, action_dim, args.init_std)
    ...
    def train(self):
        for i_episode in range(0, self.max_episode+1, self.spsa_batch*2):
            ...
            if i_episode % self.log_interval == 0 and i_episode != 0:
                disc_epi_rewards = self.evaluate(self.est_interval, 0)
                ...
                for i, dr in enumerate(disc_epi_rewards):
                    # self.writer.add_scalar('disc_reward/raw_reward', dr, i_episode+i)
                    wandb.log({'disc_reward/raw_reward': dr}, step=i_episode+i)
            ...
            disc_epi_rewards_plus = self.evaluate(self.spsa_batch, 1)
            disc_epi_rewards_minus = self.evaluate(self.spsa_batch, 2)
    ...
    def choose_action(self, state, mode=0):
        ...
        mean = actor(state)
        var = torch.diag(torch.exp(2*actor.log_std))
