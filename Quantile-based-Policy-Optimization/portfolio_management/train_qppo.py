...
from envs import PortfolioManagementEnv, VecEnv
...
class Options(object):
    def __init__(self):
        ...
        parser.add_argument('--env_name', type=str, default='PortfolioManagement')
        ...
        parser.add_argument('--lr_decay_freq', type=int, default=1e4)
...
def run(seed, device):
    ...
    env_list = [PortfolioManagementEnv(seed=100*j+seed) for j in range(args.workers)]
    env = VecEnv(env_list)
    agent = QPPO(args, env)
...
if __name__ == '__main__':
    ...
    device_list = [0 for i in range(5)]
